...
- **条件展示**: `collection/requirement.ts` 将条件区分为成绩、指定曲游玩、单局连续游玩、累计次数、多人、MISS、装扮/成员设置、玩法设置和其他特殊条件；完整原文始终保留，特殊条件明确标注为无法由最佳成绩自动判断
- **进度边界**: 仅纯成绩条件和不含流程限制的指定曲游玩可由现有成绩记录计算；只要描述包含单局过程、累计次数、多人、MISS、装扮或特殊设置要求，即使同时带有成绩门槛也不自动推断完成状态
