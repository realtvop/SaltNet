...
export function user(app: Elysia | any) {
    return (
        app
            ...
            .post(
                ...
                async ({ headers, set }: { headers: ElysiaHeaders; set: ElysiaSet }) => {
                    ...
                    const updatedSessions = sessions.filter(
                        s => s.sessionToken !== user.currentSessionToken
                    );
            ...
            .post(
                "/refresh",
                async ({ body, set }: { body: { refreshToken: string }; set: ElysiaSet }) => {
            ...
            .get(
                "/verify-email",
                async ({ query, set }: { query: { token: string }; set: ElysiaSet }) => {
                    const { token } = query;
            ...
            .post(
                "/resend-verification",
                async ({ body, set }: { body: { email: string }; set: ElysiaSet }) => {
                    const { email } = body;
