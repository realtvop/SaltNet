...
// MARK: 1. Update versions
{
    ...
    for (const version of meta.versions) {
        ...
        // 国际服更新了就也加进去
        if (
            meta.musics.some(music =>
                music.charts.some(
                    chart =>
                        (chart.regionVersionOverride?.intl || chart.version) == version.version &&
                        chart.availableRegions.includes("intl" as AvailableRegion)
                )
            )
        ) {
            versions.push({
    ...
    for (const version of versions) {
        if (
            await db.query.maimaidxVersions.findFirst({
                where: (versions, { and, eq }) =>
                    and(eq(versions.name, version.name), eq(versions.region, version.region)),
            })
        )
            continue;
...
// MARK: 2. Update musics, charts and aliases
{
    ...
    for (const music of meta.musics) {
        // Add music
        if (
            !(await db.query.maimaidxMusics.findFirst({
        ...
        for (const chart of music.charts) {
            ...
            const difficulty = {
                ...
                4: "remaster",
                10: "utage",
            }[chart.difficulty] as any;
            const chartVersions = chart.availableRegions
                .map(availableRegion => {
                    const chartVersion =
                        availableRegion === "jp"
                            ? chart.version
                            : (chart.regionVersionOverride &&
                                  chart.regionVersionOverride[availableRegion]) ||
                              chart.version;
                    ...
                    return versions.find(
                        version =>
                            version.region ==
                                { jp: "jp", intl: "ex", us: "us", cn: "cn" }[availableRegion] &&
            ...
            if (
                !(await db.query.maimaidxCharts.findFirst({
                    where: (charts, { and, eq }) =>
                        and(
        ...
        if (music.aliases && music.aliases.cn) {
            for (const alias of music.aliases.cn) {
                if (
                    !(await db.query.maimaidxMusicAliases.findFirst({
