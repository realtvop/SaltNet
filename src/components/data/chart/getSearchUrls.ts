    ...
    for (const template of VideoSearchUrls) {
        urls.push({
            ...
            url: template.url.replace(
                "%s",
                encodeURIComponent(
                    `${chart.music.info.title} ${chart.music.info.type == ChartType.Deluxe ? "DX" : template.name === "YouTube" ? "スタンダード" : "标"} ${["BASIC", "ADVANDCED", "EXPERT", "MASTER", "Re:MASTER"][chart.info.grade]} ${chart.info.level}`
                )
            ),
            icon: template.icon,
        });
